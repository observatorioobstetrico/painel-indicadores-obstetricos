...
# SINASC --------------------
# tratamento
dados_sinasc <- read.csv(gzfile("dados_oobr_indicadores_obstetricos_sinasc_1996_2024.csv.gz")) %>%
  clean_names() |>
  arrange(ano, codigo)
...
shiny::sliderInput(
  ...
  min = min(dados_sinasc$ano),
  max = max(dados_sinasc$ano),
  value = c(min(dados_sinasc$ano), max(dados_sinasc$ano)),
  sep = "",
  step = 1
)
```
...
sliderInput(
  inputId = "anosAN",
  label = HTML("<span class = 'negrito'>Selecione o intervalo de anos:</span>"),
  min = min(dados_sinasc$ano[!is.na(dados_sinasc$porc_anomalia)]),
  max = max(dados_sinasc$ano),
  value = c(min(dados_sinasc$ano), max(dados_sinasc$ano)),
  sep = "",
  step = 1
